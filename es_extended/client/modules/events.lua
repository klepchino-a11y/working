...
local function shouldWaitForIdentity()
    if Config.Multichar then return false end
    if not Config.Identity then return false end
...
-- دالة جديدة: لم تعد تغلق شاشة التحميل مباشرة.
-- الآن فقط تُعلم بقية السكربتات أن اللودنق سكرين انتهى، دون استخدام ShutdownLoadingScreen أو ShutdownLoadingScreenNui.
local function dismissLoadingScreen()
    if loadingScreenDismissed then return end
    loadingScreenDismissed = true
    -- إرسال الحدث بدون إطفاء فعلى ؛ الإغلاق يتم فى esx_loadingscreen بعد playerSpawned
    TriggerEvent('esx:loadingScreenOff')
end

local function attemptSpawn()
    if not pendingSpawn or not identityReady then
        return
    end

    local data = pendingSpawn
    pendingSpawn = nil

    ESX.SpawnPlayer(data.skin, data.coords, function()
...
local function handleIdentityReady()
    identityReady = true
    attemptSpawn()
end
...
RegisterNetEvent('westside_identity:identityReady', handleIdentityReady)
RegisterNetEvent('westside_identity:spawnPlayer', handleIdentityReady)

RegisterNetEvent('esx:playerLoaded', function(xPlayer, _, skin)
    ESX.PlayerData = xPlayer

    if not Config.Multichar then
        local waitForIdentity = shouldWaitForIdentity()
        if waitForIdentity then
            identityReady = identityReady or false
        else
            identityReady = true
        end

        pendingSpawn = { skin = skin, coords = ESX.PlayerData.coords }
        dismissLoadingScreen()
        attemptSpawn()
    end
